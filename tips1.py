...
from monads.utils import prop, destructure, eitherToTask, zipList
...
def tasks(username : str) -> Task:
    if request.method == 'POST':
        print(request.json)
        return zipList(List('usuarios', 'protocolos', 'movimientos'), #colections
                List('username', 'protocol', 'movement').fmap(prop(request.json))) \
            .fmap(lambda cn: cn[1].bind(db.get_id(cn[0]))) \
            ...
            #.bind(destructure(['userid','protocolid','movementid'], db.save_task)) \
    else:
